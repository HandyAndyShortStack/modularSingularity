...
function SVGBoard(board, chessSet, game, selector, height) {
    ...
    // defaults to attaching to the body
    ...
    // finds the center coordinates of a square
    function getCoordinates(squareId) {
        ...
        var ySide = getMissingSideLength(xSide, hypotenuse);
    ...
    // draw squares individually
    this.squares = {};
    for (var squareId in board) {
        var square = board[squareId];
        if (square.rank !== 0 && square.file + -square.rank in board) {
            
            var SWX = (w / 8) * square.fileNumber;
            var SWXSide = (w / 2) - SWX;
            var SWHypotenuse = (h / 12) * (Math.abs(square.rank) + 1);
            var SWY = getMissingSideLength(SWXSide, SWHypotenuse);
            
            var SEX = SWX + (w / 8);
            var SEXSide = (w / 2) - SEX;
            var SEHypotenuse = SWHypotenuse;
            var SEY = getMissingSideLength(SEXSide, SEHypotenuse);
            
            var NEX = SEX;
            var NEXSide = (w / 2) - NEX;
            var NEHypotenuse = SEHypotenuse - (h / 12);
            var NEY = getMissingSideLength(NEXSide, NEHypotenuse);
            
            var NWX = SWX;
            var NWXSide = (w / 2) - NWX;
            var NWHypotenuse = NEHypotenuse;
            var NWY = getMissingSideLength(NWXSide, NWHypotenuse);
            
            // SVG arc direction flags
            var flag1 = 0;
            var flag2 = 1;
            
            // reflect the black side of the board, and toggle flags
            if (square.rank < 0) {
                SWY = -SWY;
                SEY = -SEY;
                NEY = -NEY;
            ...
            // move squares into final position
            SWY += (h / 2);
            SEY += (h / 2);
            NEY += (h / 2);
            NWY += (h / 2);
            ...
            var pathString = [
                              'M', SWX, SWY, 'A', SWHypotenuse, SWHypotenuse,
                              0, 0, flag1, SEX, SEY, 'L', NEX, NEY, 'A',
                              NEHypotenuse, NEHypotenuse, 0, 0, flag2, NWX, 
                              NWY, 'L', SWX, SWY
                             ].join(' ');
               
        } else if (square.fileNumber < 4) {
            
            var x = (square.fileNumber + 1) * (w / 8);
            var xSide = (w / 2) - x;
            var hypotenuse = (h / 12) * (square.rank + 1);
            var y = getMissingSideLength(xSide, hypotenuse);
            
            var topY = -y + (h / 2);
            var bottomY =  y + (h / 2);
            
            var pathString = [
                              'M', x, bottomY, 'L', x, topY,
                              'A', hypotenuse, hypotenuse, 0, 0, 0,
                              x, bottomY
                             ].join(' ');
           
        } else {
            
            var x = (square.fileNumber) * (w / 8);
            var xSide = (w / 2) - x;
            var hypotenuse = (h / 12) * (square.rank + 1);
            var y = getMissingSideLength(xSide, hypotenuse);
